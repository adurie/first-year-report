...
\newacronym{csp}{CSP}{Cunningham Special Points}
...
\newacronym{tmr}{TMR}{tunnelling magneto-resistance}
...
\begin{document}
	...
	The following programming languages have been studied, and listed in order of fluency. Libraries studied are given in sub-entries. The references given  indicate the documentation studied.
	\begin{multicols}{2}
	\begin{itemize}
...
The following software packages have been studied with a brief description of their purposes
	\begin{itemize}
		...
		\item VIM\site{vim} \quad - \quad {\it text editor with powerful key-bindings}
			\begin{itemize}
				\item Latex-Suite \quad - \quad {\it \LaTeX package with additional key-bindings specific to \LaTeX}
			\end{itemize}
		...
		\item \LaTeX \quad - \quad {\it a powerful typesetting language}
			...
			\end{itemize}
	\end{itemize}
	Further skills developed include learning to touch-type. Miscellaneous tools used include the LLVM Clang compiler for C++, the GNU Debugger, Google gperftools and valgrind. Git\site{git} is used for version control, remote repositories are stored on the github server under the user adurie.
...
\begin{itemize}
	\item{C++}
		\begin{itemize}
			\setlength\itemsep{1em}
			\item cunningham.h \quad-\quad {\it header file that performs \gls{2dbz} integration. Further details are given in the Appendix}
			\item cunningham\_spawn.h \quad-\quad {\it header file that performs adaptive \gls{2dbz} integration. Further details are given in the Appendix}
			...
			\item spincurrent\_4-layer\_$<$j\_n-1$>$2\_keldysh.cpp \quad-\quad {\it calculates the transport term of the spincurrent (Equation \eqref{keldysh}) under the Keldysh Formalism\site{keldysh_1} in one-band \gls{sc}. The integration over energy has been bypassed by observing the first term of the Taylor expansion of the trailing Fermi terms is a delta-function. At infinitesimal bias, here $V_b = 1eV$, this first term is the only term non-negligible}
			\item spinenergy.cpp \quad-\quad {\it calculates the equilibrium term, Equation \eqref{keldysh_1} of the spincurrent under the Keldysh formalism in the one-band \gls{sc} system. Difficulties with convergence of the energy integral, as yet unresolved}
	...
	\item{Fortran}
		\begin{itemize}
			\setlength\itemsep{1em}
			\item conductor\_approx\_with\_r.f90 \quad-\quad {\it calculates the charge current for a trilayer with a conducting spacer using the Landauer formalism}
			\item insulator\_approx\_with\_$\rm r^\wedge$2.f90 \quad-\quad {\it calculates the charge current for a trilayer with an insulator spacer using the Landauer formalism}
...
This is \gls{gmr}. The so-called {\it optimistic magneto-resistance ratio} is defined by
	...
	The position vector of each atomic site is given by $\bf R$,
		...
		\end{equation}
		In order to diagonalise the matrix Hamiltonian, $\bf H$, the transformation from real space to momentum space ($k$-space) is required
		\begin{equation}\label{kkk}
			|{\bf k}\rangle = \frac{1}{\sqrt{N}}\sum_{\bf R = -\infty}^\infty e^{i{\bf k}\cdot \bf R} |\bf R\rangle
		\end{equation}
		where the summation is over every lattice point, $N$ is the total number of lattice points in the crystal. The position of any site in $k$-space is given by ${\bf K} = \sum_i^\chi k_i{\bf b}_i$ with integers $k_i$ and primitive translation vectors ${\bf b}_i$ determined such that 
		\begin{equation}
...
where since 
		...
		\end{equation}
		Assuming a small set of nearby atoms overlap with non-negligible results, all elements of the Hamiltonian are zero, except
		\begin{equation}\label{u}
			\langle{\bf R}|{\bf H}|{\bf R}\rangle = \bf u
		\end{equation}
		the on-site potential, and 
		\begin{equation}\label{t}
			\langle{\bf R}|{\bf H}|{\bf R}\pm {\bf d}\rangle = \bf t
		\end{equation}
		where $\bf d$ is the set of position vectors of the non-negligible neighbouring terms. $\bf t$ is the set of hopping matrices whose strength varies depending on the binding distance to the on-site term. Models involving multiple energy bands or multiple bases per site require that $\bf t, u$ are matrices. 
		\\[2mm]Thus
		\begin{equation}
...
\end{figure}
	In the one-band model in one dimension, all the vector values become scalar and the position of a given lattice site is $R=na$ and $d = \pm a$. As such, Equation \eqref{tight} becomes
	\begin{equation}
	...
	\begin{center}
		...
		\draw[<->,thick,>=stealth',shorten <=2pt,shorten >=2pt] (2.east)--(1.west) node[midway, above] {\scriptsize $\rm t_2$};
		\draw (1) edge [loop above, <->, thick] node {\scriptsize u} (1);
		...
		\draw[<->,thick,>=stealth, shorten <= 6pt] ({1.13}.center) -- ({r.1}.290) node[midway,above,yshift=0.2cm,xshift=-0.1cm, rotate=-64,]{\scriptsize $\rm t_1$};
	...
	\end{tikzpicture}
\end{center}
\caption{\footnotesize Depiction of an infinite \gls{bcc} lattice with lattice vector $\bf a$. The self energy is u, the nearest neighbour hopping $\rm t_1$ and the next-nearest neighbour hopping $\rm t_2$.}\label{fccchain}
\end{figure}
...
% As such, the non-zero elements of the overlap integral include Equations \eqref{t}, \eqref{u} and
		...
		% \end{equation}
		% where {\bf c} is the set of vectors separating next-nearest neighbours and {\bf s} is the next-nearest neighbour hopping.
		In the one band model, the on-site potential and hoppings are scalar.
	...
	\begin{center}
		...
		\draw[->,>=stealth,shorten <=1pt,shorten >=1pt] (2) edge [bend left] node[midway, above, yshift=-1mm] {\scriptsize $\rm u_{\scaleto{12}{2.5pt}}$} (1);
		\draw[->,>=stealth,shorten <=1pt,shorten >=1pt] (1) edge [bend left] node[midway, below, yshift=1mm, xshift=1mm] {\scriptsize $\rm u_{\scaleto{21}{2.5pt}}$} (2);
		\draw (2) edge [loop above,>=stealth, ->, min distance = 2mm, out=150] node [yshift=-1mm] {\scriptsize $\rm u_{\scaleto{11}{2.5pt}}$} (2);
		\draw (1) edge [loop above,>=stealth, ->, min distance = 1.7mm, out=95, in=40] node [yshift=-1mm] {\scriptsize u$_{\scaleto{22}{2.5pt}}$} (1);
		\draw (1) edge [loop above,>=stealth, ->, out=5, in=-20] node [left, xshift=1mm] {$\scriptstyle \rm t_{\scaleto{22}{2.5pt}}$} (5);
		\draw (1) edge [loop above,>=stealth, ->, out=15, in=-75] node [left, yshift = 2mm] {$\scriptstyle \rm t_{\scaleto{21}{2.5pt}}$} (6);
		\draw[->,>=stealth,shorten <=1pt,shorten >=1pt] (6) edge [bend left] node[midway, above, yshift=-1mm] {\scriptsize $\rm u_{\scaleto{12}{2.5pt}}$} (5);
		\draw[->,>=stealth,shorten <=1pt,shorten >=1pt] (5) edge [bend left] node[midway, below,yshift=1mm,xshift=1mm] {\scriptsize $\rm u_{\scaleto{21}{2.5pt}}$} (6);
		\draw (6) edge [loop above,>=stealth, ->, min distance = 2mm, out=150] node [yshift=-1mm] {\scriptsize u$_{\scaleto{11}{2.5pt}}$} (6);
		\draw (5) edge [loop above,>=stealth, ->, min distance = 1.7mm, out=95, in=40] node [yshift=-1mm] {\scriptsize u$_{\scaleto{22}{2.5pt}}$} (5);
		\draw (2) edge [loop above,>=stealth, ->, out=175, in=100, min distance=11mm] node [right, xshift=-1mm] {$\scriptstyle \rm t_{\scaleto{12}{2.5pt}}$}(5);
		\draw (2) edge [loop above,>=stealth, ->, out=165, in=-105] node [right, yshift = 2mm]{\scriptsize t$_{\scaleto{11}{2.5pt}}$} (6);

		\draw[->,>=stealth,shorten <=1pt,shorten >=1pt] (4) edge [bend left] node[midway, above, yshift=-1mm] {\scriptsize $\rm u_{\scaleto{12}{2.5pt}}$} (3);
		\draw[->,>=stealth,shorten <=1pt,shorten >=1pt] (3) edge [bend left] node[midway, below,yshift=1mm] {\scriptsize $\rm u_{\scaleto{21}{2.5pt}}$} (4);
		\draw (4) edge [loop above,>=stealth, ->, min distance = 2mm, out=150] node [yshift=-1mm] {\scriptsize u$_{\scaleto{11}{2.5pt}}$} (4);
		\draw (3) edge [loop above,>=stealth, ->, min distance = 1.7mm, out=95, in=40] node [yshift=-1mm] {\scriptsize u$_{\scaleto{22}{2.5pt}}$} (3);

		\draw (2) edge [loop above,>=stealth, ->, out=-75, in=-120] node [below, yshift=1mm] {$\scriptstyle \rm t_{\scaleto{11}{2.5pt}}$}(4);
		\draw (2) edge [loop above,>=stealth, ->, out=-95, in=-95, min distance = 6mm] node [below, yshift = 1mm]{\scriptsize $\rm t_{\scaleto{12}{2.5pt}}$} (3);
		\draw (1) edge [bend right,>=stealth, ->, out=0, in=100, min distance = 8mm] node [above, yshift=-1mm, xshift=-1mm] {$\scriptstyle \rm t_{\scaleto{22}{2.5pt}}$} (3);
		\draw (1) edge [bend left,>=stealth, ->, out=-30, in=210] node [above, yshift = -1mm] {$\scriptstyle \rm t_{\scaleto{21}{2.5pt}}$} (4);
	...
	\end{tikzpicture}
\end{center}
\caption{\footnotesize Depiction of an infinite square mesh of atoms in two bands separated by $a$ showing the various energies contained within the matrices in Equation \eqref{matrices}. Full details of these energies are given below.}\label{squarechain}
\end{figure}
\par	In the two band model, the interaction between electrons at different excited states occupying the two available bands per atomic site must also be considered. This means the potential of an electron associated with the on-site atom is now more complicated. Similarly, hopping to another atomic site is more complicated since there are multiple bands to hop to. Figure \ref{squarechain} indicates the origin of these terms. Nearest neighbour interactions are the only terms considered in this example, in this case Equations \eqref{u} \& \eqref{t} are the $2\times2$ matrices
\begin{equation}\label{matrices}
		\bf u=\begin{pmatrix} \rm u_{11}&\rm u_{12}\\\rm u_{21}&\rm u_{22} \end{pmatrix} \qquad,\qquad \qquad \bf t=\begin{pmatrix} \rm t_{11}&\rm t_{12}\\\rm t_{21}&\rm t_{22}\end{pmatrix}
	\end{equation}
...
\end{equation}
Returning to Dyson's equation \eqref{dyson}, which in this representation reduces to the set of equations
...
\end{equation}
Equation \eqref{rho} is calculated numerically by converting the summation into a double integral over the two dimensional Brillouin zone
\begin{equation}
...
${\bf s}_i$ is the position vector of the basis atoms. For example, \gls{bcc} has the two basis atoms at ${\bf s}_1 = (0,0,0), 	{\bf s}_2 = \frac{a}{2}(1,1,1)$, the two sites in a primitive (symmetrically irreducible) cell.
	...
	\end{tikzpicture}
\end{center}
\caption{\footnotesize The above figure shows a \gls{bcc} lattice with atomic planes stacked in the growth direction. The dashed region indicates the first Brillouin zone centred on the site at $(0,0,0)$, and the dotted region shows a primitive cell enclosing sites $a(0,-1,0)$ and $\frac{a}{2}(1,-1,1)$. It can be observed that the primitive cells tessellate completely, which forms part of their definition.}
\end{figure}
The representation in $k$-space is thus
	...
	The potential of the on-site element of the first basis atom at ${\bf s}_1$ is ${\bf u}_1$ and the potential of the on-site element of the second basis atom at ${\bf s}_2$ is ${\bf u}_2$,
	where the bold typeface of these potentials indicates they are themselves matrices in multi-band models.
	In this example, off-site terms up to second nearest neighbours are considered, where the potential induced by first nearest neighbour interactions is ${\bf t}_1$, and second nearest neighbour interactions is ${\bf t}_2$.
	...
	\end{equation}
	where $\bf d$ is the set including the position vectors of the on-site potential, the nearest neighbours and the next nearest neighbours.
...
\sum_{{\bf d}\in N_\parallel}e^{i\bf k_\parallel \cdot d}\langle{\bf 0}, 0, {\bf s}_i|{\bf H}|{\bf d}, 0, {\bf s}_i\rangle\nonumber\\&=
...
\nonumber\\&={\bf t}_1\left( 1+e^{-ik_x a}+e^{-ik_z a}+e^{-ia\left(k_x +k_z \right)}\right) 
	...
	\\[2mm]\par The elements ${\bf t}_{ii} = {\bf t}_2$. It is easy to see that the element ${\bf t}_{12}=\bf 0$ since it cannot represent nearest neighbour, or next nearest neighbour interactions. The element ${\bf t}_{21}$ has exactly the same potential as ${\bf u}_{12}^\dagger$, since the same interactions are considered.
...
This leads to the following result, derived from Dyson's equation
\begin{equation}
...
\end{equation}
Here, each element is an $N \times N$ matrix and $\bf{0}$ is a matrix of zeros. The formalism here assumes a pseudo one-dimensional crystal grown along an arbitrary axis with ${\bf v} = \bm\varepsilon - {\bf u}$, where $\bm \varepsilon$ is a matrix whose diagonal elements are the energy, $\varepsilon$, and off-diagonal elements are zero. Equation \eqref{X} in its present form and the results that follow are defined for left hand surfaces. For right-hand surfaces, values $\bf{t}$ and $\bf{t}^{\rm{\dag}}$ must be interchanged.
...
such that 
	...
	\end{equation}
	Equation \eqref{frac} has the remarkable property of demonstrating the existence of inter-atomic \gls{sgf} representations due to the continuity at non-integer n.
...
% $\bf A = (I - g_{\rm\scriptscriptstyle R}t^\dag g_{\rm\scriptscriptstyle L}t)^{\rm-1}$, $\bf B = (I -g_{\rm\scriptscriptstyle R}^\dag t^\dag g_{\rm\scriptscriptstyle L}^\dag t)^{\rm -1}$ and $\bm \sigma$ is one of the $2\times 2$ Pauli matrices $\sigma_x, \sigma_y, \sigma_z$ whose choice is dependent on the spin current axis direction to be computed. As such, the out of planespin current is required here, thus $\sigma_y$ is selected for the computation. $f(\varepsilon - \mu_\alpha)$ is the Fermi function with electrochemical potential $\mu_\alpha$. Equation \eqref{keldysh} can instead yield the total charge current if $\frac{1}{2}\bm\sigma$ is replaced by $e/\hbar $ ($\langle j_{n-1}\rangle_1$ makes no contribution to charge current)\textcolor{blue}{\textsuperscript{\cite{keldysh_2}}}.
...
% \end{figure}
	...
	Two means of calculating \gls{iec} are now given. The first is derived from the spincurrent\site{rev3}.
...
the equilibrium term, and
\begin{multline}\label{keldysh}
	\langle j_{n-1}\rangle_2 = \frac{1}{2\pi}\sum_{\bf k_\parallel}\int d\varepsilon \,\rm Re \, Tr \Big\lbrace\Big\lbrack\bf g_{\rm\scriptscriptstyle L} t A B g_{\rm \scriptscriptstyle R}^\dag t^\dag-AB+\\{\rm\frac{1}{2}} \bf(A+B)\Big\rbrack\bm\sigma\Big\rbrace [\mathnormal f(\varepsilon - \mu_{\rm\scriptscriptstyle L})-\mathnormal f(\varepsilon - \mu_{\rm\scriptscriptstyle R})]
\end{multline}
...
\begin{equation}\label{fermi}
	f(\varepsilon-\mu_{\rm\scriptscriptstyle L(R)})=\frac{1}{1+e^{(\varepsilon-\mu_{\rm\scriptscriptstyle L(R)})/kT}}
\end{equation}
...
At equilibrium, Equation \eqref{keldysh_1} yields the \gls{iec} J(n), by integrating with respect to $\theta$, from $0$ to $	\pi$
...
\end{equation}
A second method for describing \gls{iec} is derived directly from the thermodynamic potentials, which in equilibrium form is as follows\site{rev3}
...
In this communication, efforts are directed at solving Equation \eqref{exchange}, which is again very numerically intensive if attempted to solve directly. Instead, two methods to solve Equation \eqref{exchange} are outlined here. The method of summing Matsubara frequencies is described initially\textcolor{blue}{\textsuperscript{\cite{mats}}}, followed by an outline of the \acrfull{spa}\textcolor{blue}{\textsuperscript{\cite{rev1}}}.
...
has an infinite number of isolated singularities at $\varepsilon = \mu \pm (2m+1)kT\pi i$ for $m=0,1,2,\dots$, none of which lie on the real line. $kT$ is the Boltzmann constant multiplied by temperature. The semicircle lies in the upper half plane, and so all singularities at $\gamma i$ for $\gamma < 0$ are discarded. 
...
\caption{\footnotesize
	...
	$2\pi i\Big(-kT\sum_{m=0}^\infty {\rm F}(\mu{ + (2m+1)kT\pi i}, \rm\bf k_\parallel, \rm n)\Big)$. The summation converges rapidly getting very good agreement at nine terms. The contour pictured contains six of these poles.
	}\label{fig:4th}
...
\end{multline}
Where all terms not depending on $\bf k_\parallel$ have been commuted to the left of the integral in $k$-space, and where the conversion, Equation \eqref{sum} has been used. 
% \begin{equation}
% 	\frac{1}{\rm n_\parallel}\sum_{\bf k_\parallel} \longrightarrow \frac{1}{\rm A_{\scriptscriptstyle BZ}} \int_{\bf k_\parallel}d^2 \bf k_\parallel
% \end{equation}	
...
\begin{equation}\label{multivariate}
	\int_{-\infty}^{+\infty} \exp\left(\frac{i}{2}{\bf x^TAx}\right)d^2{\bf x}\equiv\int_{-\infty}^{+\infty} \exp\left(\frac{i}{2}{\sum_{j=1}^2 \lambda_j y_j^2}\right)d^2{\bf y}
	% \sqrt{\frac{(2\pi i)^n}{{\rm det}A}}
\end{equation}
where $\lambda_j$ are the two eigenvalues of $\bf A$, and ${\bf y} = {\bf O}^T \bf x$ where $\bf O$ is an orthogonal matrix obtained by diagonalising the symmetric matrix, $\bf A$ such that ${\bf D} = {\bf O}^T \bf AO$, where $\bf D$ is a diagonal matrix.
\\\par Integrating with respect to each of the independent variables on the RHS of Equation \eqref{multivariate} in turn, leads to the following result
...
\end{equation}
 ...
 \begin{flalign}\label{deta}
	\nonumber\frac{2\pi}{\sqrt{|\lambda_1 \lambda_2|}}e^{i\pi({\rm sgn}(\lambda_1)+{\rm sgn}(\lambda_2))/4}&\equiv \frac{2\pi}{|{\rm det}\left({\bf O}^T{\bf AO}\right)|^{\frac{1}{2}}}e^{i\pi({\rm sgn}(\lambda_1)+{\rm sgn}(\lambda_2))/4}\\\nonumber
	&\equiv\frac{2\pi\tau}{|{\rm det}\left({\bf O}^T{\bf AO}\right)|^{\frac{1}{2}}}\\
	&\equiv\frac{2\pi\tau}{|{\rm det}\left({\bf A}\right)|^{\frac{1}{2}}}
\end{flalign}
...
\end{equation}
whereupon inspection of the left hand fraction within the integrand, given that it varies slowly with respect to $\varepsilon$ and lies entirely on the real line, $\varepsilon$ may be replaced by its first order term
...
\normalsize
\\[5mm]
which is further simplified since $k_x, k_z = 0$. The results comparing the method of Matsubara, and \gls{spa} are presented in Figure \ref{fig:3rd}.
\\ [5mm]
...
\\\par By perturbing quantities during modelling, and isolating terms during certain summations within Equation \eqref{spa} in the one-band model, it became apparent the phase was dominated by the first Fourier term (using direct complex Fourier series expansion against \gls{sgf}s $\bf g$, this term is $c_{-1}$), in particular since this term is complex, it is the phase $\psi_s$ of the Fourier term that is of importance.
...
\end{equation}
with ${\bf o}_1, {\bf o}_2$ denoting the upper-left and upper-right sub-matrices of $\bf O$, and $\bm \tau$ is the hopping matrix of the lead,
...
More details are given in the Appendix.
	...
	In his 1974 paper\textcolor{blue}{\textsuperscript{\cite{CP}}}, Cunningham describes the special points set that minimises the required function calls for converged integration of the \acrfull{2dbz}, for a number geometries. His work follows that of Chadi and Cohen\textcolor{blue}{\textsuperscript{\cite{CC}}}, where the integrable function is expanded as a Fourier series
...
\\[2mm]\par It is easy to generalise this technique to the square lattice, where if a point is sampled on the line $k_x = k_z$ it is halved, and only five further child points are generated, since the further three do not lie within the irreducible segment. Two of these also lie on the line $k_x = k_z$, which means they are also weighted by a half. When spawning further points, only the parents that lie on the line $k_x = k_z$ spawn children that must also adhere to this special case. Otherwise the nine points as outlined in the previous paragraph are sampled.
...
\begin{tikzpicture} 
	\begin{axis}[
		...
		\addplot[only marks, mark size=0.7pt,violet] table [col sep=comma] {qti3.txt};
	\end{axis}
\end{tikzpicture}
\caption{\footnotesize
