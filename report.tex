...
This is \gls{gmr}. The so-called {\it optimistic magneto-resistance ratio} is defined by
	...
	It is assumed there is one atom per basis and that the effect of one atom on another is small. The wavefunction of each isolated atom is $\psi(\bf R-R_{\mathnormal j})$, where $\bf R, R_{\mathnormal j}$ are the position vectors of the atomic sites. Then the approximate wavefunction for a crystal of $N$ atoms is
		\begin{equation}
			\Psi_{\bf k}({\bf R}) = \frac{1}{\sqrt{N}}\sum_j e^{i{\bf k\cdot R}_j}\psi({\bf R-R}_j)
		\end{equation}
		...
		At this stage the transformation from real space to Hilbert space is made
...
Where 
		...
		\end{equation}
		Assuming \gls{nn} overlap only, all elements of the Hamiltonian are zero, except
		\begin{equation}\label{t}
			\langle{\bf R}|{\bf H}|{\bf R}\pm {\bf d}\rangle = \bf t
		\end{equation}
		where $\bf d$ is the set of vectors separating \gls{nn}'s, $\bf t$ is the hopping integral and 
		\begin{equation}\label{u}
			\langle{\bf R}|{\bf H}|{\bf R}\rangle = \bf u
		\end{equation}
		is the onsite potential. Thus 
		\begin{equation}
			\langle{\bf R}|{\bf H}|{\bf \acute{k}}\rangle = \frac{1}{\sqrt{N}}\sum_{\bf \acute{R}}\langle{\bf {R}}|{\bf H}|{\bf \acute{R}}\rangle e^{i\bf \acute{k}\cdot \acute{R}}= \frac{1}{\sqrt{N}}e^{i\bf \acute{k}\cdot R}\left({\bf u}+\sum_{{\bf d }\in {\rm nn}}e^{i{\bf k \cdot d}}\langle{\bf R}|{\bf H}|{\bf R}\pm {\bf d}\rangle\right)
		\end{equation}
		and
		\begin{equation}\label{sumtb}
			\langle{\bf k}|{\bf H}|{\bf \acute{k}}\rangle = \frac{1}{N}\sum_{\bf R} e^{i\bf R\cdot (\acute{k}-k)}\left( {\bf u + t}\sum_{{\bf d}\in {\rm nn}}e^{i{\bf k \cdot d}}\right) = \delta_{\bf k \acute{k}}\left( {\bf u + t}\sum_{{\bf d}\in {\rm nn}}e^{i{\bf k \cdot d}}\right)
		\end{equation}
		where Equation \eqref{kk} is used.
		The energy eigenvalues are thus
		\begin{equation}
			\varepsilon({\bf k})= {\bf u + t}\sum_{{\bf d}\in {\rm nn}}e^{i{\bf k \cdot d}}
			\label{tight}
		\end{equation}
...
\end{figure}
	...
	\end{equation}
	\subsubsection{One-band model \gls{bcc} example}
...
As such, the non-zero elements of the overlap integral include Equations \eqref{t}, \eqref{u} and
		...
		\par A general procedure for satisfying Equation \eqref{ab} in 3D is as follows\textsuperscript{\textcolor{blue}{\cite{datta}}}
		\begin{equation}
		...
		\end{gather*}
		\par and {\bf c} is the set of vectors 
		\begin{equation*}
	...
	\begin{center}
		\begin{tikzpicture}[scale=3, every node/.style={scale=1.5}]
			\draw[shading=ball, opacity=0.4] (0,0) circle (3mm);
			\draw[shading=ball, opacity=0.4] (1,0) circle (3mm);
		...
		\node[circle,fill=black, inner sep=1pt] (4) at (0.9,0){};
			...
			\draw[shading=ball, opacity=0.4] (0,1) circle (3mm);
		...
		\draw (2) edge [loop above,>=stealth, ->, min distance = 2mm, out=150] node [yshift=-1mm] {\scriptsize u} (2);
		\draw (1) edge [loop above,>=stealth, ->, min distance = 1.7mm, out=95, in=40] node [yshift=-1mm] {\scriptsize u} (1);
		\draw (1) edge [loop above,>=stealth, ->, out=5, in=-20] node [left, xshift=1mm] {$\scriptstyle \rm t^\dagger$} (5);
		...
		\draw (6) edge [loop above,>=stealth, ->, min distance = 2mm, out=150] node [yshift=-1mm] {\scriptsize u} (6);
		\draw (5) edge [loop above,>=stealth, ->, min distance = 1.7mm, out=95, in=40] node [yshift=-1mm] {\scriptsize u} (5);
		...
		\draw (4) edge [loop above,>=stealth, ->, min distance = 2mm, out=150] node [yshift=-1mm] {\scriptsize u} (4);
		\draw (3) edge [loop above,>=stealth, ->, min distance = 1.7mm, out=95, in=40] node [yshift=-1mm] {\scriptsize u} (3);
		...
		\draw (1) edge [bend right,>=stealth, ->, out=0, in=110, min distance = 8mm] node [above, yshift=-1mm] {$\scriptstyle \rm t^\dagger$} (3);
...
\end{figure}
\par	As in the one band model, the Hamiltonian must be diagonalised. For nearest neighbour only interactions, Equations \eqref{t}, \eqref{u} may be used which in this case are the $2\times2$ matrices
	\begin{equation}
		\bf u=\begin{pmatrix} \rm u&s\\s&\rm u \end{pmatrix} \qquad,\qquad \qquad \bf t=\begin{pmatrix} \rm t&\tau\\\tau^\dagger&\rm t\end{pmatrix}
	\end{equation}
	with $s$ being the energy required to raise (lower) an electron to a more (less) excited state on the same atom and $\tau$ is the hopping to an excited state on a neighbouring atom. In real space, the Hamiltonian is now block diagonalised
	\begin{equation}
	...
	\begin{equation}
		\mathcal{H} = \begin{pmatrix} \bf u&{\bf t}\sum_{\bf d}e^{i\bf k \cdot d}& \bf 0&\bf 0\\{\bf t}^\dagger\sum_{\bf d} e^{-i\bf k \cdot d}&\bf u &\ddots&\bf 0\\\bf 0&\ddots&\ddots&{\bf t}\sum_{\bf d} e^{i\bf k \cdot d}\\\bf 0&\bf 0&{\bf t}^\dagger\sum_{\bf d} e^{-i\bf k \cdot d}&\bf u\end{pmatrix}
	\end{equation}
...
Summing all elements and scaling as $1/N$ yields
	...
	\begin{equation}
		\rm det \begin{pmatrix} \rm u+2t\cos \left(\bf k_\parallel{\mathnormal a} \right)-\varepsilon(\bf k) &s+2\tau
			\cos \left( {\bf k_\parallel}a \right) \\ s+2
			\tau^\dagger\cos \left({\bf k_\parallel}a \right) &\rm u+2t\cos \left( \bf k_\parallel {\mathnormal a}
\right)-\varepsilon(\bf k) \end{pmatrix} =0
 ...
 % \left(\cos \left( k_x\mathnormal a \right)+\cos \left( k_z\mathnormal a \right)\right) +{\mathnormal s}^{2}}}
	...
	\begin{multline}
		\varepsilon(\bf k) =\rm u+2t\left(\cos \left( k_x{\mathnormal a} \right)+ \cos \left( k_z{\mathnormal a} \right) \right)
 ...
 \left(\cos \left( k_x\mathnormal a \right)+\cos \left( k_z\mathnormal a \right)\right) +{\mathnormal s}^{2}}}
	\end{multline}
	Which upon switching off $s, \tau$ yields the one-band case, as expected.
...
\end{equation}
The representation perpendicular to the surface plane is atomiclike and thus the mixed Bloch-Wannier form is used
\begin{equation}
...
\end{equation}
Where $\delta_{\bf k,\acute{k}}$ is the Kronecker-Delta which is equal to $1$ when $\bf k = \acute{k}$, and $0$ otherwise. Here the result $\langle {\bf k}|H^0|{\bf \acute{k}} \rangle = E^0 ({\bf k}) \delta_{\bf k,\acute{k}}$ has been used, where
\begin{equation}
...
\end{equation}
The substitution $z=e^{i{\rm k}_\perp a}$ is made, so that $\cos({\rm k}_\perp a) = \frac{1}{2}\big(e^{i{\rm k}_\perp a}+e^{-i{\rm k}_\perp a}\big)= \frac{1}{2}\big(z +\frac{1}{z}\big)$.\par Also $\frac{dz}{d{\rm k}_\perp}=iaz , \quad d{\rm k}_\perp =\frac{dz}{iaz},$ thus integrating over the unit circle $C$ yields
\begin{equation}
...
\rho_m(\varepsilon)=\frac{-1}{\pi N_\parallel}\sum_{\bf k_\parallel} \Im\left\{ G(m,m,{\bf k_\parallel})\right\}.
\end{equation}
...
\par The region $-2\rm t \leq \varepsilon \leq 2t$ is some periodic function that flattens out the larger the value $n$ chosen, whereas the region $\varepsilon<-2\rm t, \varepsilon>2t$ appears to be almost parabolic. The on-site energy $\rm u$ displaces the whole function along the $\varepsilon$ axis. The full width of the function is 12t.
...
\begin{equation}
	\bf{g_{\rm 1}} = (\bf{v}-\bf{t}^{\dag}\bf{g_0t})^{\rm -1}
\end{equation}
...
\end{figure}
\par The representation in $k$-space is thus
	\begin{equation}
		|{\bf k}_\parallel, {\bf k}_\perp, {\bf s}\rangle = \frac{1}{\sqrt{N_\parallel}}\sum_{\bf R_\parallel = -\infty}^\infty e^{i{\bf k}_\parallel\cdot {\bf R}_\parallel} |{\bf R}_\parallel,{\bf R}_\perp, {\bf s} \rangle
	\end{equation}
...
\\\par The definition of an $N$-dimensional M\"{o}bius transformation is as follows, let
\begin{equation}
...
\begin{equation}
\begin{aligned}
\lim_{\varepsilon \rightarrow \mu + (2m+1)kT\pi i}\frac{1}{1+e^{(\varepsilon-\mu)/kT)}}=\\\lim_{\varepsilon \rightarrow \mu + (2m+1)kT\pi i}\frac{1}{\frac{1}{kT}e^{(\varepsilon-\mu)/kT}}=-kT
\end{aligned}
\end{equation}
Thus
...
% The Green's function for the spacer layer are obtained by growing from the LHS lead using Equation \eqref{adlayer}. This is in order to maintain maximum numerical efficiency given that calculations of previous layers are stored in the process.
	...
	The \gls{spa} method relies on the fact that F($\varepsilon, \rm \bf k_\parallel, \rm n)$ is a quasi-periodic function of the continuous thickness n. As such, F($\varepsilon, \rm \bf k_\parallel, \rm n)$ may be expanded as a Fourier series
...
\end{multline}
As stated above, the essence of \gls{spa} is to assume the contributions are largest when the phase is stationary. Thus ${\bf k}_\parallel^0$ is the solution to ${\partial \phi_s}/{\partial{\bf k}_\parallel } = 0$. Thus
...
\begin{equation}
	{\rm J(n)} = -\frac{(2\pi)^2{kT}}{2{\rm nA_{\scriptscriptstyle BZ}}}\sum_s \sum_{{\bf k}_\parallel^0}{\rm Re}\left.\left(\frac{\tau  c_se^{i{\rm n}\phi_s}}{\vert {\rm det}(\partial^2\phi_s)\vert^\frac{1}{2}\sinh(\pi kT[n\phi_s '+\psi_s '])} \right) \right|_{\mu , {\bf k}_\parallel^0}
\end{equation}
\subsubsection{1-band Simple Cubic model}
	...
	$(\partial^2 \phi_s)$ is the $2\times 2$ Hessian matrix $(\partial^2\phi_s)_{x, z} \equiv \partial^2\phi_s/\partial{\rm \bf k_{\parallel \mathnormal x}}\partial{\rm \bf k_{\parallel \mathnormal z}}$ evaluated at the stationary point $\rm \bf k_\parallel^{\rm 0}$.
...
\\\par It is the first term that dominates the phase of \gls{iec}, $c_1$
\begin{equation}
...
More details are given in the Appendix.
	...
	\\\par For these simple lattice types, the integral is performed as follows;
	\begin{equation}
	...
	\\
	\par The simplest of these two techniques is as follows: 
